#!/usr/bin/env python3
"""BOO! star-purchase IPN server (battleoforigins.com/play → Wyatt's PayPal).

Receives PayPal Instant Payment Notifications for Boo! Spirit Battles
purchases, verifies them with PayPal's postback endpoint, and credits the
buyer's Firebase RTDB profile (testroom-75200, boo2/players/{uid}).

Clone of the proven FAVOR rail (/opt/favor-ipn) with the boo2 namespace,
its own port, and one new pack kind: the Founder's Lantern non-consumable
(grants no stars — sets players/{uid}/founders=true instead).

Client side: js/payments.js WebProvider builds a _xclick checkout with
invoice <boo2Uid>.<packKey>.<yyyyMMddHHmmss> and
notify_url https://nationgame.live/api/boo/paypal/ipn.

Pack table must match js/payments.js PRODUCTS exactly (short keys in the
invoice, NOT the reverse-DNS StoreKit ids):
  boo.stars100=$0.99  boo.stars550=$4.99  boo.stars1200=$9.99
  boo.founders=$4.99 (founders flag, 0 stars)

Stdlib only — no pip dependencies. Run behind nginx (proxy /api/boo/).
"""
import json
import logging
import os
...
LISTEN_ADDR = ("127.0.0.1", 8713)
FIREBASE_BASE = "https://testroom-75200-default-rtdb.firebaseio.com/boo2"
...
# key: (stars, usd, grants_founders)
PACKS = {
    "boo.stars100": (100, "0.99", False),
    "boo.stars550": (550, "4.99", False),
    "boo.stars1200": (1200, "9.99", False),
    "boo.founders": (0, "4.99", True),
}

# uid = 'u' + Math.random base36 + Date.now base36 — see play/js/meta.js uid().
# persona rows (rival_*) can never match.
INVOICE_RE = re.compile(r"(u[a-z0-9]{10,30})\.(boo\.[a-z0-9]+)\.(\d{14})")

STATE_DIR = os.environ.get("STATE_DIR", "/var/lib/boo-ipn")
...
def claim_txn(txn_id: str, record: dict) -> bool:
    """Create boo2/purchases/{txn} exactly once (ETag CAS on absent node).
...
def grant_founders(uid: str):
    """Founder's Lantern: idempotent non-consumable flag on the player row."""
    with fb_request(f"players/{uid}/founders", "PUT", True):
        pass


def queue_congrats(uid: str, stars: int, item_id: str, txn_id: str, founders: bool):
    """The client's msgQueue drain shows a celebration on next visit."""
    import time
    with fb_request(f"players/{uid}/msgQueue", "POST", {
        "type": "star_purchase", "stars": stars, "item": item_id,
        "founders": founders, "txn": txn_id, "at": int(time.time() * 1000),
    }):
        pass
...
class Handler(BaseHTTPRequestHandler):
    server_version = "BooIPN/1.0"

    def log_message(self, fmt, *args):
        log.info("http: " + fmt % args)

    def do_GET(self):
        if self.path == "/api/boo/paypal/health":
            self._respond(200, "ok")
        else:
            self._respond(405, "POST only")

    def do_POST(self):
        if self.path != "/api/boo/paypal/ipn":
    ...
    def process(self, raw: bytes):
        ...
        if item_id not in PACKS:
            log.warning("REJECT: unknown item %s", item_id)
            return
        stars, expected_usd, founders = PACKS[item_id]
        ...
        with _lock:
            ...
            new_balance = grant_stars(uid, stars) if stars else None
            if founders:
                grant_founders(uid)
            ...
            if new_balance is not None:
                with fb_request(f"purchases/{txn_id}/balance", "PUT", new_balance):
                    pass
            try:
                queue_congrats(uid, stars, item_id, txn_id, founders)
            except Exception:
                log.exception("msgQueue congrats failed (grant already landed)")

            processed.add(txn_id)
            save_processed(processed)
            log.info("GRANTED: +%d stars%s to %s (txn %s, %s) — balance %s",
                     stars, " +founders" if founders else "", uid, txn_id, item_id, new_balance)


def main():
    log.info("BOO IPN server starting on %s:%s (sandbox allowed: %s)",
